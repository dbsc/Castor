...
fn crasher() {
    ...
    let link_line = format!("{}_{} = {} | {}\n", alias, file_name, manifest_hash, final_dir);
...
fn restorer() {
    ...
    for line in links_content.lines() {
        if line.contains(clean_hash) {
            ...
            if parts.len() == 2 {
                auto_dir = parts[1].trim().to_string();
            }
        }
    }

    let manifest_path = format!("{}{}.manifest", auto_dir, clean_hash);
    ...
    for hash in chunk_hashes.iter() {
        let chunk_path = format!("{}{}", auto_dir, hash);
    ...
    for line in links_content.lines() {
        if line.contains(clean_hash) {
            let main_part = line.split(" | ").next().unwrap_or("");
    ...
    fs::write(&final_save_path, final_file).expect("Failed to save file");
    println!("DONE! File restored successfully as '{}'!", final_save_path);
}
